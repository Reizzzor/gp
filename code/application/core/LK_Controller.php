...
class LK_Controller extends CI_Controller
{
	...
	protected function make_sticky_table($tbl_id, $th_arr, $rows, $y_offset = NULL, $class = 'table_resize')
	{
		...
		$tbl_template = array('table_open' => "<table class='$class' id='$tbl_id'>");
	...
	protected function make_orders_table($orders, $t_name = 'Информация о заказах: ')
	{
		...
		foreach ($orders as $order) {
			...
			if (isset($order['summ_total'])) {
				if (!$this->data['s_info']['hide_fin_part'] && $order['debt']) {
					$debt = $this->out_summ(($order['summ_total'] - $order['debt']) / 100) . ' руб.';
				} else {
					$debt = '-';
				}
			} else {
				$debt = 'сумма не найдена';
			}
			$ords_rows[] = [$order['sln_order_id'], $order['order_id'], $order['nomenclature'], $order['quantity'], $order_status, $this->get_yes_or_no($order['pay']), $this->get_yes_or_no($order['is_podium']), $this->date_format($order['kept_stamp']), $this->date_format($order['ready_stamp']), $this->date_format($order['out_stamp']), $has_drp, $drp_stamp, $debt];
		}
		$ords_tbl = $this->make_sticky_table('orders_table', ["№ в салоне", "Фабр. №", "Номенклатура", "Кол-во", "Статус", "Оплата", "Подиум. образец", "Дата принятия", "Дата готовности", "Дата отгрузки", "Наличие ткани", "Дата прихода", "Задолженность"], $ords_rows, 145);
